...
+ (void)start {
    ...
    [self changeCacheCountLimit:INT_MAX costLimit:1024 * 1024 * 20 ageLimit:DBL_MAX];
}
...
static NSString * kOurRecursiveRequestFlagProperty = @"kOurRecursiveRequestFlagProperty";
//以下这个链接为host的网址不做处理
...
{
    if (response != nil) {//有重定向  先请求 回来一个response如果是重定向 浏览器要再一次请求
